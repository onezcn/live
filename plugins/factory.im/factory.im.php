...
/* ========================================================================
 * $Id: factory.im.php 21555 2020-05-07 11:25:59Z onez $
 * 
 * Email: www@onez.cn
 * QQ: 6200103
...
class onezphp_factory_im extends onezphp{
  ...
  */
  function init($opt){
    global $G;
    $uniqidFile=ONEZ_CACHE_PATH.'/imkey.lock';
    if(!$uniqidFile){
      $uniqid=uniqid();
      onez()->write($uniqidFile,$uniqid);
    }
    $uniqid=onez()->read($uniqidFile);
    $this->siteId=md5(__FILE__."\t".$G['this']->token."\t$uniqid\t".$G['this']->get_setting('imkey'));
